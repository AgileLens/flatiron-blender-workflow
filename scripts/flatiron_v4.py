"""Flatiron build-4 geometry: reference-guided facade with a support contract.
...
# ---------------------------------------------------------------- parameters
LOT_FIFTH, LOT_22ND = 197.5 * FT, 86 * FT
R_SW, R_SE, R_PROW = 1.9, 1.7, 1.05          # corner radii; prow ~2.1 m wide
COLUMN_SPACING = {'Fifth': 17 * FT, 'Broadway': 18.5 * FT, '22nd': 16 * FT}
PAIRS = {'Fifth': 9, 'Broadway': 9, '22nd': 4}
ENTRANCE_PAIRS = {'Fifth': [4], 'Broadway': [4], '22nd': [0, 3]}
ORIEL_PAIRS = {'Fifth': [1, 4, 7], 'Broadway': [1, 4, 7]}
WW, NP = 1.42, 0.85                            # sash width, narrow pier within a pair
RECESS, EMBED = 0.30, 0.02                     # glass plane depth, mount overlap
D0 = -(RECESS + EMBED)                          # back of piers/spandrels (inside core)
Z = {1: 0.0, 2: 5.6, 3: 9.8, 4: 14.0, 5: 18.1}
for _n in range(6, 19):
    Z[_n] = 18.1 + (_n - 5) * 3.95             # 5th-17th stories (13 ft)
Z[19] = Z[18] + 4.1; Z[20] = Z[19] + 4.1; Z[21] = Z[20] + 3.65   # arcade, 20th, cornice base
...
def footprint(inset=0.0, segs=(10, 10, 14), trim_prow=0.0):
    ...
    C = [(-W / 2, -L / 2), (W / 2, -L / 2), (-W / 2, L / 2)]   # SW, SE, prow (north)
    ...
    for side in ('Fifth', 'Broadway'):
        for k in ENTRANCE_PAIRS[side]:
            ...
            for x in (c - half - .27, c + half + .27):
                ...
                cylinder(LIME, (px + nn[0] * .1, py + nn[1] * .1), .34, 0.0, 4.42, tag='engaged column')
            pblock(LIME, path, c - half - .75, c + half + .75, -.05, .45, 4.4, 5.35, 'entablature')
            for x in (c - 1.3, c, c + 1.3):
                roundel(LIME, path, x, 4.87, .2, .43, .52, tag='entablature roundel')
    building_rings(path)
...
def building_rings(path):
    ...
    rings = [  # (z0, z1, projection, material)
        (4.5, 4.75, .2, LIME), (4.75, 5.25, .12, LIME), (5.25, 5.45, .36, LIME), (5.45, 5.75, .58, LIME),
...
def blender_main(out):
    ...
    bpy.ops.wm.save_as_mainfile(filepath=os.path.join(out, 'flatiron_v4.blend'))


if __name__ == '__main__':
    argv = sys.argv[sys.argv.index('--') + 1:] if '--' in sys.argv else sys.argv[1:]
    ap = argparse.ArgumentParser(); ap.add_argument('--out', default='outputs/build4'); ap.add_argument('--dry', action='store_true')
    ...
    if not args.dry:
        ...
        with open(os.path.join(args.out, 'flatiron_v4-stats.json'), 'w') as fh:
            json.dump(st, fh, indent=2)
        print('SAVED', os.path.join(args.out, 'flatiron_v4.blend'), flush=True)
